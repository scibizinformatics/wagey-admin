...
// Get company ID from route, store, or auth
const companyId = ref(1) // This should be dynamic
...
async function fetchAttendanceData(params = {}) {
  loading.value = true
  try {
    const response = await api.get(`/attendance/company/${companyId.value}/`, {
      params: {
...
async function fetchEmployeeDetails() {
  try {
    const response = await api.get(`/user/employees/${companyId.value}/`)
    const employeeData = response.data.data || response.data
    employees.value = employeeData

    employeeOptions.value = employeeData.map((emp) => ({
...
function getEmployeeName(employee) {
  ...
  if (employee) {
    const emp = employees.value.find((e) => e.id === employee)
    if (emp && emp.user) {

...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      error: null,
      apiEndpoint: 'https://staging.wageyapp.com/payroll/company/payslips/',
    ...
    async fetchPayrollData() {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          this.payrollData = this.processPayrollData(data)
        } else if (data.data && Array.isArray(data.data)) {
          this.payrollData = this.processPayrollData(data.data)
        } else {
          this.payrollData = this.processPayrollData([data])
        }
      } catch (err) {
